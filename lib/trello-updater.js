...
function constructJSON(textArray) {
  var obj = {}
...
export default {
  ...
  exportToTrello() {
    // Sample Comment
    // board:Malaysia,list:To do,name:Test2,dueDate:2018-07-20,desc:TestDesc
    var text = atom.workspace.getActiveTextEditor().getSelections()[0].getText();
    text = text.replace('//','');
    var textArray = text.split(',');
    var textObject = constructJSON(textArray);
    ...
    getBoardIdFromName(textObject.board).then(function(boardId) {
      getListIdFromBoard(textObject.list, boardId).then(function(listId) {
        ...
        createCard(textObject).then(function(attachments) {
          atom.notifications.addSuccess(`Card added to list '${textObject.list}' on board '${textObject.board}'`);
        }, function(err) {
          console.log(err);
